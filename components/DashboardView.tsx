...
const DashboardView: React.FC = () => {
  ...
  // Mocked Period-based Financial Data (stored in Cents)
  const financialData = useMemo(() => {
    const data = {
      today: { revenue: 450000, expenses: 120000, income: 410000, liabilities: 8500000, receivables: 3400000, prevRevenue: 420000 },
      week: { revenue: 3250000, expenses: 1850000, income: 2950000, liabilities: 8200000, receivables: 4100000, prevRevenue: 2800000 },
      month: { revenue: 14500000, expenses: 6200000, income: 13800000, liabilities: 7500000, receivables: 5600000, prevRevenue: 13900000 },
      custom: { revenue: 0, expenses: 0, income: 0, liabilities: 0, receivables: 0, prevRevenue: 0 }
    };
    return data[period];
  }, [period]);
  ...
  const costSplit = [
    { label: 'Ingredients & Goods', value: 45, cents: Math.round(financialData.expenses * 0.45), color: 'bg-emerald-500' },
    { label: 'Staff Hub (Salary/SC)', value: 30, cents: Math.round(financialData.expenses * 0.30), color: 'bg-indigo-500' },
    { label: 'Operational (Rent/Utils)', value: 15, cents: Math.round(financialData.expenses * 0.15), color: 'bg-amber-500' },
    { label: 'Marketing', value: 10, cents: Math.round(financialData.expenses * 0.10), color: 'bg-rose-500' },
  ];

  const salesVsIncomePoints = [
    { day: 'Mon', sales: 450, income: 420 },
    { day: 'Tue', sales: 520, income: 500 },
    { day: 'Wed', sales: 480, income: 485 },
    { day: 'Thu', sales: 610, income: 590 },
    { day: 'Fri', sales: 750, income: 700 },
    { day: 'Sat', sales: 890, income: 850 },
    { day: 'Sun', sales: 400, income: 380 },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
          ...
          {(['today', 'week', 'month', 'custom'] as Period[]).map((p) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                period === p ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {p}
            </button>
        ...
        <div className="glass rounded-[2.5rem] p-10 space-y-8">
           ...
           <div className="p-6 bg-slate-950/50 rounded-3xl border border-slate-800 space-y-4">
              ...
              <p className="text-xs font-medium text-slate-400 leading-relaxed italic">
                Inventory and Staffing account for 75% of your total spend. Consider auditing ingredient waste in the next cycle.
              </p>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="glass rounded-[2.5rem] p-10 border-l-4 border-rose-500 space-y-6">
            ...
            <div className="space-y-1">
               <h5 className="text-4xl font-black text-white tracking-tighter">{formatCurrency(financialData.liabilities)}</h5>
               <p className="text-xs font-bold text-slate-500">Current Payables + Outstanding Loans</p>
            </div>
         ...
         <div className="glass rounded-[2.5rem] p-10 border-l-4 border-indigo-500 space-y-6">
            ...
            <div className="space-y-1">
               <h5 className="text-4xl font-black text-white tracking-tighter">{formatCurrency(financialData.receivables)}</h5>
               <p className="text-xs font-bold text-slate-500">Pending Settlements + Customer Credit</p>
            </div>
            <div className="pt-6 border-t border-slate-800">
